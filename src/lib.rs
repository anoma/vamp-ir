...
pub fn setup_cmd(Setup { max_degree, output, unchecked }: &Setup) {
    // Generate CRS
 ...
 */
pub fn compile_cmd(Compile { universal_params, source, output, unchecked }: &Compile) {
 ...
 * inputs. */
pub fn prove_cmd(Prove { universal_params, circuit, output, unchecked }: &Prove) {
...
pub fn verify_cmd(Verify { universal_params, circuit, proof, unchecked }: &Verify) {
    println!("* Reading arithmetic circuit...");
    let mut circuit_file = File::open(circuit)
        .expect("unable to load circuit file");
